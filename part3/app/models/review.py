...
class Review(BaseModel):
    ...
    def update(self, data):
        ...
        # Handle place and user updates with business logic validation
    ...
    def to_dict(self):
        ...
        # Convert place and user objects to IDs for API representation
        if hasattr(self, 'place') and self.place:
            review_dict['place_id'] = self.place.id
            del review_dict['place']  # Remove the full place object
        
        if hasattr(self, 'user') and self.user:
            review_dict['user_id'] = self.user.id
            del review_dict['user']  # Remove the full user object
        
        return review_dict
    
    def __str__(self):
        """String representation of the Review."""
        return f"[Review] ({self.id}) Rating: {self.rating}/5 by {self.user.first_name}"
